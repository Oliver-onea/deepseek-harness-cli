...
describe('parseDshArgs', () => {
  it('boots the terminal profile when the invocation names none', () => {
    expect(parse([])).toEqual({ mode: 'profile', profile: 'tui', patches: [], args: [] })
    // Everything the launcher does not own is the terminal app's own argument,
    // which is what makes `dsh "<task>"` and `dsh --resume <id>` work.
    expect(parse(['run', 'the', 'tests']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: [], args: ['run', 'the', 'tests'] })
    expect(parse(['--resume', 'session-x']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: [], args: ['--resume', 'session-x'] })
    expect(parse(['--patch', 'a.yml', '--resume', 'session-x']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: ['a.yml'], args: ['--resume', 'session-x'] })
  ...
  it('rejects empty values and contradictory inputs', () => {
