...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // `cordis/*` are runtime event names, not package references; rescoping them
  // would break the producer/consumer contract because the type declaration and
  // emit sites keep the bare name.
  ...
  // `cordis` is the UI locale namespace for the Cordis panel and its cards;
  // the `PropsLocale<'cordis'>` type parameter and `t('cordis')` lookup name
  // the dictionary, not the framework package.
  ...
  // `cordis` is the event-scope key used to route generated catalog pages;
  // it names the cordis-events section, not the framework package.
