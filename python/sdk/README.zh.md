...
`Session` 同时支持交互式 steering 与 interrupt：
...
`Session.steer(input)` 向运行中的 turn 发送 steering 内容并返回拼接后的消息 id。`Session.interrupt(keep_inbox=True)` 终止活跃 turn 并保留排队工作；默认 `keep_inbox=False` 会清除排队和 steering inbox 项。
