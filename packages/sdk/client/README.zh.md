...
自有运行 API 之下的协议客户端：显式 `start()`/`initialize()`/`prompt()`/`interrupt()`/`request()`/`close()`，外加通知订阅。`prompt()` 在运行时接受排队消息后立即返回该消息的 ID，绝不等待 agent 活动。`interrupt(sessionId, { keepInbox? })` 与上述会话句柄捷径语义相同。`subscribe(filter?)` 返回 `NotificationSubscription`（可等待的 `next()`、非阻塞 `tryNext()`、异步迭代）；`subscribeSessionTree(id)` 把范围限定到一个会话及从 `subagent.started` 血缘边发现的后代——运行时对上下文内每个会话都发通知，范围限定在客户端完成，与 Python SDK 完全一致。本包导出有明确类型的错误：`JsonRpcResponseError`（协议错误响应，保留 code/data）、`RequestTimeoutError`（配置的时限已到）、`SdkProtocolError`（响应超出文档化协议）、`TransportClosedError`（运行时已消失——消息携带退出码与有界 stderr 尾部）。
...
- **Python SDK 差距**——`interrupt` 尚无 Python 对应方法；设计孪生将随其自身改动获得（见协议的[已知限制](../protocol/README.md)）。
