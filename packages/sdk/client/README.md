...
`session(id).steer(input)` splices steering content into that session: a running turn consumes it at its next step boundary, an idle session opens its next turn with it, and a session the runtime does not know rejects with the wire error naming the id. It returns the spliced message id; the consumption itself is observed through the notification stream.

`session(id).interrupt({ keepInbox? })` aborts that session's active turn: queued and steering work is discarded unless `keepInbox: true` preserves it (preserved work stays parked until a later waking prompt claims it), an idle session accepts the interrupt as a no-op, and a session the runtime does not know rejects with the wire error naming the id. The abort itself is observed through the notification stream, not the return value.

Approval questions arrive as the protocol's one server→client request: `onApproval` on the constructor options (or `client.onApprovalRequest(handler)` at the protocol layer) installs the handler that decides them. Without a handler the runtime's questions get an error response and the tool call fails closed; a throwing handler, and a malformed runtime answer, fail closed the same way. The handler receives the session id, tool name, call id, and asker reason, and `{ outcome: 'allowed-once' }` is the only grant it can return. The `approval/asked`/`approval/decided` audit pair rides the `session.event` stream like any other event.

## HarnessClient

The protocol client under the owned-run API: explicit `start()`/`initialize()`/`prompt()`/`steer()`/`interrupt()`/`request()`/`close()`, plus notification subscriptions. `prompt()` returns the queued message id as soon as the runtime accepts it; it never waits for agent activity. `steer(sessionId, contentBlocks)` and `interrupt(sessionId, { keepInbox? })` carry the same semantics as the session-handle shortcuts above. `subscribe(filter?)` returns a `NotificationSubscription` (awaitable `next()`, non-blocking `tryNext()`, async iteration); `subscribeSessionTree(id)` scopes to one session and the descendants discovered from `subagent.started` lineage edges — the runtime notifies for every session in its context, and scoping is client-side, exactly like the Python SDK. Error surfaces are typed and exported from this package: `JsonRpcResponseError` (wire error response, code/data preserved), `RequestTimeoutError` (a configured bound elapsed), `SdkProtocolError` (a response outside the documented protocol), `TransportClosedError` (the runtime is gone — message carries the exit code and a bounded stderr tail).
...
- **No per-prompt result** — low-level `prompt()` returns only an enqueue receipt and high-level `run()` owns receipt-to-idle collection; `interrupt` stops a session's work without attributing an outcome to one prompt.
- **Python SDK parity gap** — `steer` and `interrupt` have no Python counterpart yet, and only the TypeScript client answers `approval/request`; the design twin gains each with its own change (see the protocol's [Known Limitations](../protocol/README.md)).
- **Client→server notifications are unimplemented** on both wire ends; the transport carries them for future flows.
