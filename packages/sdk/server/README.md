...
`initialize.serverInfo.name` is the wire-stable `deepseek-harness-sdk-runtime`. An optional positive `initialize.maxTokens` becomes the request output cap of each SDK-created agent and its in-process descendants; invalid values reject initialization, while omission sends no SDK cap and allows the selected adapter or provider route default to apply. `session/prompt` queues one identified user message and immediately returns `{ messageId }`. `session/steer` splices one steering message into a known session's `next-step` inbox through its live agent (`Agent.steer`): a running turn consumes it at its next step boundary, an idle session opens its next turn with it, and an unknown session id fails with an error naming it — steer never creates the session. `session/interrupt` aborts the named session's active turn through its live agent (`Agent.cancel({ kind: 'user' })`) and returns `{}` once accepted: queued and steering work is discarded unless `keepInbox: true` preserves it (preserved work stays parked until a later waking prompt claims it), an idle session accepts the interrupt as a no-op, and an unknown session id fails with an error naming it.

When the surrounding composition mounts an approval service, the plugin also answers its `approval/request` waterfall for its own agents: the question travels to the wire client as the protocol's one server→client request (`approval/request`), carrying the session id, tool name, call id, and asker reason. The client's `{ outcome: 'allowed-once' }` is the only grant; every other answer, an error response, and an unanswered question fail closed through the approval service's containment (`'unavailable'` or `'rejected'`). An aborting tool call withdraws the question (`'cancelled'`) via the request's abort signal. Questions for agents this server does not own delegate down the chain.

The server streams every durable fact as `session.event` and every whole-agent lifecycle transition as `session.status`; it does not assign an assistant message or `turn/end` to that prompt. Independent requests may enqueue more work on the same session. Persistence roots and persona come from `cordis.yml`.
...
- **The wire has no per-session close method** — `session/interrupt` stops work, but SDK-created agents remain live until process shutdown.
