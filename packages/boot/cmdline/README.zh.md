...
- **启动器的 flag 必须写在应用参数之前**：切分按位置进行，启动器不认识的第一个 token 就是内层参数的起点，因此写在某个应用 flag 之后的 `--patch` 属于应用。分界前的 `--` 会转发给应用解析器，分界后的 `--` 本来就属于应用；两种位置下都由应用消费它，并将后续形似 flag 的 token 当作位置参数。
