...
| `StartupRefusalError` / `classifyStartupFailure(error)` | 标记用户可修正的启动失败，并在进程边缘对捕获的失败分类；分类器沿标准 `Error.cause` 链查找标记，为单行报告返回其消息，并保留每个未标记错误的完整崩溃堆栈 |
...
插件或持久输入读取器可以在用户可通过修正调用或配置解决启动问题时抛出 `StartupRefusalError`。启动包装器可以通过 `Error.cause` 增加上下文；`classifyStartupFailure` 沿该链查找标记，让进程边缘把标记消息作为一行 stderr 输出。未标记失败保留原始错误和堆栈，使内部崩溃仍可调试。
