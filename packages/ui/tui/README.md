...
| `images` | `true` | Whether to attempt inline image rendering at all; `false` always draws an image's text fallback, regardless of terminal capability. |
| `imageMaxWidthCells` | `60` | Maximum inline image width in terminal cells. |
| `imageMaxHeightCells` | — | Maximum inline image height in terminal cells; unset keeps the image's own aspect ratio. |
...
The transcript is folded from the **append-origin session log**, not from the model-visible surface, so a resumed session keeps every message the reader already saw and a compacted range stays readable behind its marker. It renders human turns behind a `›` marker and assistant turns behind a colored `◆` marker, assistant text as Markdown, reasoning (hidden by default), tool cards, and one-line notices for turn endings, settled commands, and compactions; each human or assistant turn opens a blank line so turn boundaries read as spacing, while the editor frame keeps the only horizontal rules. Rendered lines are cached per entry and invalidated by content or width change, so redrawing a long conversation costs only the entries that moved.

Tool cards come from each tool's own `presentCall`/`presentResult` ([render intent](../../core/tools/README.md)): terminal, diff, read, search, and web cards each have a terminal form, and a card shape this renderer does not know falls back to the model-facing result. A tool changes how it reads here by changing its presenter, never by a branch in this package. The card opens with the call line — the presenter's title, else `name(args)` with one identifying argument — and a settled result attaches beneath it on a `╰` elbow with a dimmed body, the elbow in the error color when the call failed; no border surrounds the card.

An image the reader attached draws inline, through pi-tui's own `Image` component: its `getCapabilities()` probe decides the protocol (Kitty or iTerm2 graphics) from the live terminal's own environment, never from a deployment setting, and a terminal that answers with neither — a plain xterm, most `tmux`/`screen` sessions without Kitty passthrough, or an SSH hop that lost the protocol's `TERM` — draws the same text fallback pi-tui uses everywhere else (media type, pixel dimensions, and file name, when known). The read for a capable terminal's actual bytes runs through `ctx.attachments`, off the render path: the entry draws its fallback immediately from the durable reference's own metadata, and only replaces it once the background read settles, so a slow or failing attachment store degrades to the fallback rather than blocking the screen. `images: false` skips the probe and the read entirely, drawing every image as its fallback regardless of what the terminal could do — a fixed deployment choice for a proxy or recording pipeline that should never receive graphics escapes. Because the image lives inside the scrolling transcript rather than a fixed-height overlay, it does not hide below a row gate the way the input-trigger and model-picker menus do: pi-tui's own scroll accounting crops a placed Kitty image at the viewport edge the same way it crops a long tool card or assistant message, so a 20-row-tall image on a five-row terminal scrolls into view instead of vanishing. `imageMaxWidthCells`/`imageMaxHeightCells` bound how large one placement can request. Only user-authored images draw this way today — see Known Limitations for the forward-compatible block this does not yet cover.

The footer reports run state — a spinner while a turn runs, a green dot while idle, ahead of the elapsed time and token count — the route requests actually use (the latest `request/context`, restated immediately when `/model` switches ahead of one), context occupancy from [`ctx.tokenMeter`](../../llm/token-meter/README.md), the latest `todo/write` plan, queued inbox depth, the controls that currently apply, and standing session-state indicators when the composition mounts them: the current goal (except completed goals), an active or pending plan mode, and the effective permission preset. It pins no standing key hint: the interrupt hint rides the running turn, and the input row carries the rest.

The editor's input row opens with a `›` prompt marker; while the input is empty a dim placeholder names what the reader can do, and the gutter narrows to the marker once typing or a menu begins, dropping away entirely on a terminal too narrow to keep the editor usable beside it.
...
Every string reaching pi-tui or the pane title passes through `displayText()`, which collapses carriage returns, expands tabs, and renders every other C0/DEL/C1 control as a visible `\xNN` escape. Only this package and pi-tui create ANSI control sequences, so untrusted tool output or model text cannot repaint the screen, move the cursor, or set the pane title. Inline images are the one place this package hands pi-tui raw bytes instead of `displayText()`-normalized text — pi-tui's `Image` component base64-encodes them into a graphics-protocol placement sequence itself, so an attacker cannot smuggle an escape sequence through image bytes: the encoding admits only the pixel data, never arbitrary control bytes.
...
| `/help` | List the commands the live registry resolves for this agent. |
...
- **Only user-authored images draw inline** — an assistant or tool-result image block (the `read_image` tool's result, forward-compatible assistant output) still draws no picture; `ImageBlock` is role-neutral, but no current adapter emits assistant images and `read_image`'s card is `generic` with no image-capable presenter yet, so both remain text-only until a current producer needs otherwise.
- **No multiplexer- or SSH-specific probing** — capability detection is pi-tui's `getCapabilities()` reading the live process environment once; a Kitty-capable local terminal reached through a multiplexer or remote hop that does not forward the protocol's `TERM` falls back the same as a genuinely incapable terminal, and there is no separate remote-aware heuristic.
...
- **The prompt gutter sits beside the editor, not inside it** — pi-tui's editor exposes no placeholder or prompt-prefix hook, so the `›` marker and the empty-input placeholder draw in a gutter next to the border and the border reflows once when the placeholder gives way to typing; ghost text inside the input area would need a change to pi-tui itself.
