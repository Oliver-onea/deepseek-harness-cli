...
The footer reports run state — a spinner while a turn runs, a green dot while idle, ahead of the elapsed time and token count — the route requests actually use (the latest `request/context`, restated immediately when `/model` switches ahead of one), context occupancy from [`ctx.tokenMeter`](../../llm/token-meter/README.md), the latest `todo/write` plan, queued inbox depth, the controls that currently apply, and standing session-state indicators when the composition mounts them: the current goal (except completed goals), an active or pending plan mode, and the effective permission preset.
