...
- `/` at the start of the input lists the commands the **live** [`ctx.commands`](../../../../packages/interaction/commands/README.md) registry resolves for the driven agent. The provider reads `commands.list(agent)` on every query, so a command registered after the screen is up appears on the next keystroke and a shadowed one disappears, without a restart.
...
- A picked `/` candidate completes to the command line and submits through the registry, so it never reaches the model. A picked `@` candidate inserts the reference `@name ` into the input, which ships to the model verbatim — the web surface's own plain-text pick arm; agent-side consumption of `@` references is future business work on both surfaces.
...
The `/` menu lists commands grouped only by the registry's name order, without the web surface's source-grouped headings, and no skill source is composed, so only commands appear under `/`; the web surface's skill source has no terminal counterpart yet.
