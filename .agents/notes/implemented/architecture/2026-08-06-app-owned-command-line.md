...
The launcher parses only what it owns — `--profile`, `--patch`, the config dumps — and hands **everything after its own flags** to the booted tree verbatim. The split is positional: the first token the launcher does not recognize starts the app's arguments (commander's `passThroughOptions` + `allowUnknownOption` + `helpOption(false)`). A `--` at the launcher boundary is restored as the first app argument, while one after the boundary is already in the verbatim suffix; in either position the app parser owns and consumes the terminator. A bare `dsh -h`, which has no app to hand the flag to, still prints the launcher's own help.
...
- Launcher flags must precede app arguments; a first app argument equal to `web` or `plugin` selects that subcommand instead, and `-V`/`--version` remains launcher-owned before that boundary. A `--` on either side of the boundary reaches the app parser, which consumes it and treats following flag-shaped tokens as positional arguments.
